...
def _handle_two_digits(num_text: str) -> str:
    if num_text[0] == "0":
        return digits_dict[num_text[1]]
    elif num_text[0] == "1":
        return ten_to_nineteen_dict[num_text]
    else:
        return " ".join([dozens_dict[num_text[0]], _handle_one_digit(num_text[1])])


def _handle_three_digits(num_text: str) -> str:
    hundred = "hundred"
    return " ".join([_handle_one_digit(num_text[0]), hundred, _handle_two_digits(num_text[1:])])
...
def num_to_eng(n: int) -> str:
    ...
    if digits_count == 1:
        ...
        return _handle_two_digits(num_text)

    elif digits_count == 3:
        return _handle_three_digits(num_text)
